...
async function buildSegmenter(device, onProgress) {
  ...
  return pipeline('image-segmentation', MODEL_ID, {
    ...
    progress_callback: (event) => {
      ...
      onProgress?.({ status: event?.status || 'loading', file: event?.file || '', percent, device })
    },
  })
}
...
function normalizeMask(raw) {
  const data = raw?.data
  if (!raw || !data || !raw.width || !raw.height) throw new Error('A IA retornou uma máscara inválida.')
...
function mergeMasks(masks) {
  ...
  for (const mask of masks) {
    for (let i = 0; i < merged.length; i += 1) if (mask.data[i] > merged[i]) merged[i] = mask.data[i]
  }
...
export async function segmentFloor(imageUrl, onProgress) {
  ...
  const normalizedFloorMasks = floorSegments
    .sort((a, b) => (b.score || 0) - (a.score || 0))
    .map((item) => normalizeMask(item.mask))

  const floorMask = mergeMasks(normalizedFloorMasks)
  const occlusionSegments = outputs.filter((item) => isOccluder(item.label) && (item.score || 0) > 0.18)
  const occlusionMasks = occlusionSegments.map((item) => normalizeMask(item.mask))
  const occlusionMask = mergeMasks(occlusionMasks)
  const bestScore = floorSegments.reduce((max, item) => Math.max(max, item.score || 0), 0)
  ...
  return {
    ...
    device: activeDevice,
    occlusionMask,
    detectedObjects: occlusionSegments.map((item) => item.label).slice(0, 12),
  }
}
